local scriptVersion = "0.5" -- do not edit this unless you know what you're doing
...
local specListSliderX = gui.Slider(guiBoxOtherInformation, "thisisatest1", "thisisatest1", 25, 0, screenWidth)
local specListSliderY = gui.Slider(guiBoxOtherInformation, "thisisatest2", "thisisatest2", 660, 0, screenHeight)
...
callbacks.Register("Draw", function()
	if spectatorList:GetValue() then
		...
		if #spectators ~= 0 then
			draw.Color(0, 0, 20, 100)
			draw.FilledRect(specListX, specListY-5, specListX+200, specListY+#spectators*15+5)
			draw.ShadowRect(specListX, specListY-5, specListX+200, specListY+#spectators*15+5, 2)
			draw.Color(57, 108, 255, 255)
			...
			for i, player in pairs(spectators) do
				...
				draw.Color(255, 255, 255, 255)
				local name = ""
				if client.GetPlayerInfo(player:GetIndex()).fakeplayer then
					name = "BOT "
				end
				name = name .. player:GetName()
				draw.TextShadow(specListX+8, specListY-12+i*15, name)
			end
			if input.IsButtonDown(1) then
...
local guiBoxOtherMovement = gui.Groupbox(otherTab, "Movement Settings", 305, 185, 285, 50)
