gui.Command("clear")
local scriptVersion = "0.7" -- do not edit this unless you know what you're doing
...
elseif http.Get("https://raw.githubusercontent.com/BlueElixir/aimware-luas/main/moonlight/ver.txt") == "404: Not Found" then
	print([[
Unable to fetch version information. Please PM user 404765 (xBlue, moonlight developer) on the forums with this:
...
local displayTogglesColours = {
	gui.ColorPicker(displayToggles, "togglesListWindowNameColour", "Toggles List Window Name Colour", 255, 255, 255, 255),
	gui.ColorPicker(displayToggles, "togglesListNameColour", "Toggles List Name Colour", 57, 108, 255, 255)
}
	...
	if scriptToggle:GetValue() and displayToggles:GetValue() then
		...
		if displayTogglesStyles:GetValue() == 0 then
			...
			if entities.GetLocalPlayer() ~= nil or gui.Reference("Menu"):IsActive() then
				...
				draw.Color(57, 108, 255, 255)
				draw.Color(displayTogglesColours[2]:GetValue())
				...
				draw.Color(displayTogglesColours[1]:GetValue())
						...
						if gv(miscOtherVariables[1]) and gv(miscOtherVariables[3]) then
							if gv(miscOtherVariables[7]) ~= nil and gv(miscOtherVariables[7]) ~= 0 then
								if input.IsButtonDown(gv(miscOtherVariables[7])) then
									if triggerMagnetToggle:GetValue() then
										state = "magnet"
										draw.Color(unpack(indicatorItems.toggleColours[3]))
									else
										state = "on"
		...
		elseif displayTogglesStyles:GetValue() == 1 then
			...
			if entities.GetLocalPlayer() ~= nil or gui.Reference("Menu"):IsActive() then
				...
				for i=2, #indicatorItems.toggleList, 1 do
...
local guiBoxOtherImportant = gui.Groupbox(otherTab, "Important Information", 10, 455, 285, 50)
...
suggestions @ BlueElixir#0985]])

local guiBoxOtherBuybot = gui.Groupbox(otherTab, "Buy Bot", 305, 349, 285, 50)
...
--		hasbought = 1
--	else
--		print("Already purchased this round.")
--	end
end
--[[
callbacks.Register("FireGameEvent", function(e)
