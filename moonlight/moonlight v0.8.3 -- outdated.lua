...
local function cacheChams()
	for i=0, #chamsTogglesEnemy/2-1 do
		chamsTogglesEnemy[i*2+2] = gv(chamsTogglesEnemy[i*2+1])
	end
	for i=0, #chamsTogglesOther/2-1 do
		chamsTogglesOther[i*2+2] = gv(chamsTogglesOther[i*2+1])
	end
	chams_cached = true
end
...
local function chamsDisable()
	for i=0, #chamsTogglesEnemy/2-1 do
		sv(chamsTogglesEnemy[i*2+1], 0)
	end
	for i=0, #chamsTogglesOther/2-1 do
		sv(chamsTogglesOther[i*2+1], 0)
	end
...
local function chamsReturn()
	for i=0, #chamsTogglesEnemy/2-1 do
		sv(chamsTogglesEnemy[i*2+1], chamsTogglesEnemy[i*2+2])
	end
	for i=0, #chamsTogglesOther/2-1 do
		sv(chamsTogglesOther[i*2+1], chamsTogglesOther[i*2+2])
	end
	chams_returned = true
end
	...
	if scriptToggle:GetValue() and drawToggles:GetValue() then
		if entities.GetLocalPlayer() ~= nil or gui.Reference("Menu"):IsActive() then
			...
			for i=1, #indicatorItems.toggleList, 1 do
				...
				elseif i == 4 then
					local temp = 0
					if not espEnableOnDeath:GetValue() then
...
local guiBoxEspHealth = gui.Groupbox(espTab, "Healthbar Settings", 305, 375, 285, 30)
...
callbacks.Register("Draw", function()
	if scriptToggle:GetValue() and enableWorldModulation:GetValue() then
		...
		if enableFogModulation:GetValue() then
			...
			if client.GetConVar("fog_start") ~= fogStart:GetValue() then
				client.SetConVar("fog_start", fogStart:GetValue(), true)
			end
			if client.GetConVar("fog_end") ~= fogEnd:GetValue() then
				client.SetConVar("fog_end", fogEnd:GetValue(), true)
			end
			if client.GetConVar("fog_maxdensity") ~= fogDensity:GetValue() then
				client.SetConVar("fog_maxdensity", fogDensity:GetValue(), true)
			end
			if client.GetConVar("fog_color") ~= tempfc then
				client.SetConVar("fog_color", tempfc, true)
			end
			if client.GetConVar("fog_colorskybox") ~= tempsfc then
				client.SetConVar("fog_colorskybox", tempsfc, true)
			end
			if client.GetConVar("fog_startskybox") ~= skyboxFogStart:GetValue() then
				client.SetConVar("fog_startskybox", skyboxFogStart:GetValue(), true)
			end
			if client.GetConVar("fog_endskybox") ~= skyboxFogEnd:GetValue() then
				client.SetConVar("fog_endskybox", skyboxFogEnd:GetValue(), true)
			end
			if client.GetConVar("fog_maxdensityskybox") ~= skyboxFogDensity:GetValue() then
...
callbacks.Register("Draw", function()
	if spectatorList:GetValue() and scriptToggle:GetValue() then
...
callbacks.Register("Draw", function()
	if scriptToggle:GetValue() == false then
		guiBoxLegitMain:SetDisabled(true)
		guiBoxLegitBacktrack:SetDisabled(true)
		guiBoxLegitTriggerbot:SetDisabled(true)
		guiBoxEspMain:SetDisabled(true)
		guiBoxEspOther:SetDisabled(true)
		guiBoxEspHealth:SetDisabled(true)
		guiBoxEspWorld:SetDisabled(true)
		hudOptions:SetDisabled(true)
	else
		guiBoxLegitMain:SetDisabled(false)
		guiBoxLegitBacktrack:SetDisabled(false)
		guiBoxLegitTriggerbot:SetDisabled(false)
		guiBoxEspMain:SetDisabled(false)
		guiBoxEspOther:SetDisabled(false)
		guiBoxEspHealth:SetDisabled(false)
		guiBoxEspWorld:SetDisabled(false)
