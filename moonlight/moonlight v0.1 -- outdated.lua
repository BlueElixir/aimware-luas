local scriptVersion = "0.1" -- do not edit this unless you know what you're doing
...
callbacks.Register("Draw", function()
	if scriptToggle:GetValue() and displayMoonlightWatermark:GetValue() then
		...
		local val = cheat.GetUserName()
		local temp = draw.GetTextSize(val)
		...
		draw.TextShadow(screenWidth-90-temp, 25, "moonlight | " .. val)
	end
end)
...
espTabToggle:SetDescription("Toggle ESP settings. Recommended other bind.");
...
espEnableOnDeath:SetDescription("Coming in v0.2!")
espEnableOnDeath:SetDisabled(true)
...
callbacks.Register("Draw", function()
	if espTabToggle:GetValue() == true then
		...
		if sharedEsp:GetValue() == true then
...
local espSettingsItems = {
	...
	gui.Checkbox(espFlagToggles, "espOverlayDefuser", "Has Defuser", false),
		...
		if espTabToggle:GetValue() then
			for i=1, #espMainVariables, 1 do
				if espSettingsItems[i]:GetValue() then
					if i == 4 or i == 16 then
						...
						gui.SetValue(espMainVariables[i], true)
					end
				end
			end
		else
			for i=1, #espMainVariables, 1 do
				if i == 4 or i == 16 then
					...
					gui.SetValue(espMainVariables[i], false)
				end
			end
		end
	end
...
local aspectRatioChangerValue = gui.Slider(guiBoxOtherMain, "aspect_ratio_reference", "Aspect Ratio", 177, 1, 200)
callbacks.Register("Draw", function()
...
More features coming in v0.2, maybe v0.3.
