...
local guiRef = gui.Reference("Visuals", "Overlay", "Weapon")
local newToggle = gui.Checkbox(guiRef, "newToggle", "Better Bomb Indicators", true)
newToggle:SetDescription("Change the indicator position.")
...
function DrawingHook()
	...
	if planting == true then
		...
		if newToggle:GetValue() then
			draw.TextShadow(ScreenW/100, ScreenH/2, bombsite .. " - Planting")
		else
		...
		draw.SetFont(font2)
		if newToggle:GetValue() then
			draw.TextShadow(ScreenW/100, ScreenH/2-20, planter .. " - " .. PlantTime .. "s")
		else
	...
	if entities.FindByClass("CPlantedC4")[1] ~= nil then
		...
		local bombtimer = math.floor((plantedat - globals.CurTime() + Bomb:GetProp("m_flTimerLength")) * 10) / 10
			...
			if defusing == true then
				...
				draw.SetFont(font1)
				if newToggle:GetValue() then
					draw.TextShadow( ScreenW/100, ScreenH/2, bombsite .. " - " .. bombtimer .. "s");
				else
				...
				draw.SetFont(font2)
				if newToggle:GetValue() then
					draw.TextShadow(ScreenW/100, ScreenH/2-20, "Defusing - " .. defusetime .. "s")
				else
			...
			if Player:IsAlive() and globals.CurTime() < Bomb:GetProp("m_flC4Blow") then
				...
				if hpleft >= Player:GetHealth() then
					...
					if newToggle:GetValue() then
						draw.TextShadow(ScreenW/100, ScreenH/2 + 25, "FATAL");
					else
				...
				elseif hpleft <= 0 then return
					...
					if newToggle:GetValue() then
						draw.TextShadow(ScreenW/100, ScreenH/2 + 25, "-" .. hpleft .. " HP");
					else
